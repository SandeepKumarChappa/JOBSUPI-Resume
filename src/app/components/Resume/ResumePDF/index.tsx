...
import { ResumePDFPortfolio } from "components/Resume/ResumePDF/ResumePDFPortfolio";
...
export const ResumePDF = ({
  ...
  const { profile, workExperiences, educations, projects, skills, custom, portfolio } =
    resume;
  const { name } = profile;
  const {
    fontFamily,
  ...
  const formTypeToComponent: { [type in ShowForm]: () => JSX.Element } = {
    ...
    portfolio: () => (
      <ResumePDFPortfolio
        heading={formToHeading["portfolio"]}
        portfolio={portfolio}
        themeColor={themeColor}
        isPDF={isPDF}
      />
  ...
  const renderModern = () => {
    const rightForms = showFormsOrder.filter((form) => form !== "skills");
    const leftHasSkills = formToShow.skills;
    return (
      <Page
        size={documentSize === "A4" ? "A4" : "LETTER"}
        style={{
  ...
  return (
    <>
      <Document title={`${name} Resume`} author={name} producer={"JOBSUPI Resume"}>
        {pageByTemplate}
      </Document>
